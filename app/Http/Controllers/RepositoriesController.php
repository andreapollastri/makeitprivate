...
class RepositoriesController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        try {
            $response = Http::withToken(User::first()->github_token)->get('https://api.github.com/repos/'.$repo->source.'/tags');
        ...
        foreach ($tags as $tag) {
            try {
                $response = Http::withToken(User::first()->github_token)->get('https://api.github.com/repos/'.$repo->source.'/contents/composer.json?ref='.$tag->name);
        ...
        try {
            $response = Http::withToken(User::first()->github_token)->get('https://api.github.com/repos/'.$repo->source.'/branches');
        ...
        foreach ($branches as $branch) {
            try {
                $response = Http::withToken(User::first()->github_token)->get('https://api.github.com/repos/'.$repo->source.'/contents/composer.json?ref='.$branch->name);
                $content = json_decode($response->getBody()->getContents());
                $composer = json_decode(file_get_contents($content->download_url));
    ...
    {
        if ($request->token != crc32(config('app.key'))) {
            abort(403);
        }
